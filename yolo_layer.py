    ...
    def forward(self, output, target=None):
        if self.training:
            ...
            anchor_w = torch.Tensor(self.anchors).view(
                nA, self.anchor_step).index_select(1, torch.LongTensor([0])).cuda()
            anchor_h = torch.Tensor(self.anchors).view(
                nA, self.anchor_step).index_select(1, torch.LongTensor([1])).cuda()
            ...
            if False:
                ...
                print("             total : %f" % (t4 - t0))
            print("%d: nGT %d, recall %d, proposals %d, loss: x %f, y %f, w %f, h %f, conf %f, cls %f, total %f" % (self.seen, nGT, nCorrect,
                                                                                                                    nProposals, loss_x.data[0], loss_y.data[0], loss_w.data[0], loss_h.data[0], loss_conf.data[0], loss_cls.data[0], loss.data[0]))
            return loss
        else:
            masked_anchors = []
            ...
            masked_anchors = [
                anchor / self.stride for anchor in masked_anchors]
